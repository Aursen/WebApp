...
function saveCanvas() {
    save.innerHTML = "<b>Sauvegardé: </b>En cours..";
    saveBtn.disabled = true;
    canvas.toBlob(blob => {
        const formData  = new FormData();
        formData.append("image_file", blob);

        fetch(`/save/${username.value}`, {
            method: 'POST',
            body: formData
        }).then(res => {
            if(res.ok) {
                saveBtn.disabled = false;
                save.innerHTML = "<b>Sauvegardé: </b>Oui";
            }else{
                saveBtn.disabled = false;
                save.innerHTML = "<b>Sauvegardé: </b>Oups erreur..";
            }
        });
    });
}
    ...
    conn.onmessage = function (e) {
        ...
        switch (msg[0]) {
            case "canvas":
                ...
                save.innerHTML = "<b>Sauvegardé: </b>Non";
                break;
        }
